...
import { ArrowLeft, Heart, Plus, X, Flame, DollarSign } from 'lucide-react';
...
export default function MangaDetailPage() {
  ...
  useEffect(() => {
    if (manga) {
      // Set default purchase price to the advised max buy price
      setPurchasePrice(manga.maxBuyPrice.toString());
    }
  }, [manga]);
  ...
  const defaultMarginMin = manga.minSellPrice - manga.maxBuyPrice;
  const defaultMarginMax = manga.maxSellPrice - manga.maxBuyPrice;
  const defaultROIMin = manga.maxBuyPrice > 0 ? (defaultMarginMin / manga.maxBuyPrice) * 100 : 0;
  const defaultROIMax = manga.maxBuyPrice > 0 ? (defaultMarginMax / manga.maxBuyPrice) * 100 : 0;
  const retailDiscountPct = manga.retailPrice > 0 ? ((manga.retailPrice - manga.maxBuyPrice) / manga.retailPrice) * 100 : 0;

  // Real-time calculations based on user input
  const currentPaid = parseFloat(purchasePrice) || 0;
  const realMarginMin = manga.minSellPrice - currentPaid;
  const realMarginMax = manga.maxSellPrice - currentPaid;
  ...
  const advice = getAdvice(currentPaid, manga.maxBuyPrice, realROIMin);
      ...
      <div className="detail-grid" style={detailGridStyle}>
        ...
        <div style={leftColStyle}>
          <div className="glass-panel" style={{ padding: '24px', textAlign: 'center' }}>
            <MangaCover title={manga.title} series={manga.series} volumeRange={manga.volumeRange} height={340} />
            
            <div style={{ marginTop: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={retailBadgeStyle}>Neuf en magasin : {manga.retailPrice.toFixed(2)} €</span>
              <button
        ...
        <div style={rightColStyle}>
          <div className="glass-panel" style={panelPaddingStyle}>
            <div style={{ borderBottom: '1px solid var(--border-medium)', paddingBottom: '16px', marginBottom: '20px' }}>
              <span style={seriesSubStyle}>{manga.series}</span>
              <h1 style={titleStyle}>{manga.title}</h1>
            </div>

            {/* Primary Advisor KPIs */}
            <div style={kpiGridStyle}>
              <div className="glass-panel" style={kpiCardStyle('var(--accent-neon-glow)')}>
                ...
                <span style={kpiValueStyle}>{manga.maxBuyPrice.toFixed(2)} €</span>
              ...
              <div className="glass-panel" style={kpiCardStyle('rgba(0, 230, 118, 0.15)')}>
                ...
                <span style={kpiValueStyle}><span style={{ color: 'var(--success)' }}>{manga.minSellPrice.toFixed(2)} €</span></span>
              ...
              <div className="glass-panel" style={kpiCardStyle('rgba(0, 136, 255, 0.15)')}>
                ...
                <span style={kpiValueStyle}><span style={{ color: 'var(--accent-blue)' }}>{manga.maxSellPrice.toFixed(2)} €</span></span>
          ...
          <div className="glass-panel" style={panelPaddingStyle}>
            <h3 style={sectionTitleStyle}>
              ...
              Calculateur d&apos;Achat Réel
            </h3>
            <p style={sectionSubStyle}>Saisissez le prix négocié sur Vinted pour simuler votre bénéfice en temps réel.</p>
            
            <div className="calc-grid" style={calcGridStyle}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                <label style={calcLabelStyle}>Prix d&apos;achat Vinted payé (€)</label>
            ...
            <div style={disclaimerBoxStyle}>
              Analyse indicative : les prix du marché peuvent varier selon l&apos;état, la demande et le moment de la vente.
            </div>
          </div>
          ...
          <div className="glass-panel" style={panelPaddingStyle}>
            ...
            {/* Simulated Google Trends Line Graph */}
